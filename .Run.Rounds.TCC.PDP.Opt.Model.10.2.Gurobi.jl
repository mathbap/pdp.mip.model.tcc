...
function PDP_C19(m::Model, lks::Tuple, cr::Array{Int64}, t_dict::Dict )
    for (i,j) in lks
        if i != j
            for k in cr    
                    @constraint(m, ( ( m[:td][i,k] + t_dict[i,j] - m[:ta][j,k] ) <= ( (5000) * (1 - m[:x][i,j,k]) ) ) ) end
        end
    end
    println("c19.ok")
end; 
...
function PDP_C23(m::Model, cr::Array{Int64}, tb_cr::DataFrame)
    for k in cr
        @constraint(m, (m[:ta][tb_cr[k,3],k] - m[:td][tb_cr[k,2],k]) <= (528))
    end
...
#-------------------------------------------------------obj
function PDP_SOL(m::Model, id::Int64)
    open(string(id,".Round.log.file.opt.txt"), "w") do file write(file, "optimal solution = ", string(JuMP.objective_value(m))) end
    open(string(id,".Round.log.file.opt.txt"), "a") do file write(file,"
", "------------------------------------------------","
")end
end; 

#-------------------------------------------------------w
function PDP_VRW(m::Model, rt::Array{Int64}, id::Int64)
    for r in rt
        Rw = JuMP.value.(m[:w][r]) 
        if Rw == 1 (
        open(string(id,".Round.log.file.opt.txt"), "a") do file write(file,"
...
#-------------------------------------------------------x
function PDP_VRX(m::Model, nd::Array{Int64}, cr::Array{Int64}, id::Int64)
    for k in cr
        for i in nd
            for j in nd
...
#-------------------------------------------------------y
function PDP_VRY(m::Model, nd::Array{Int64}, cr::Array{Int64}, rt::Array{Int64}, id::Int64)
    for r in rt
        for k in cr
            for i in nd
                for j in nd 
...
#-------------------------------------------------------z
function PDP_VRZ(m::Model, nd::Array{Int64}, cr::Array{Int64}, id::Int64)
    for k in cr
        for i in nd
            for j in nd
...
#-------------------------------------------------------ta
function PDP_VTA(m::Model, nd::Array{Int64}, cr::Array{Int64}, id::Int64)
        for k in cr
            for i in nd
...
#-------------------------------------------------------ta
function PDP_VTD(m::Model, nd::Array{Int64}, cr::Array{Int64}, id::Int64)
    for k in cr
        for i in nd
...
#-------------------------------------------------------search
function TUN_SEARCH(m::Model)
    Gurobi.GRBtunemodel(backend(m))
    return println("Tuning.Search.ok")
end;

#-------------------------------------------------------result
function TUN_RESULT(m::Model)
    Gurobi.GRBgettuneresult(backend(m),0)
    return println("Tuning.Result.ok")
end;

#-------------------------------------------------------write
function TUN_WRITE(m::Model)
    Gurobi.GRBwrite(backend(m),string(id,".Round.log.file.opt.prm"))
    return println("Tuning.Write.ok")
end;
...
#---------------------------------------------------------
SOL(model, id)  =  @time PDP_SOL(model, id);
VRW(model, requests, id)  =  @time PDP_VRW(model, requests, id);
VRX(model, nodes, cars, id)  =  @time PDP_VRX(model, nodes, cars, id);
VRY(model, nodes, cars, requests, id)  =  @time PDP_VRY(model, nodes, cars, requests, id);
VRZ(model, nodes, cars, id) =  @time PDP_VRZ(model, nodes, cars, id);
VTA(model, nodes, cars, id)  =  @time PDP_VTA(model, nodes, cars, id);
VTD(model, nodes, cars, id)  =  @time PDP_VTD(model, nodes, cars, id);
#---------------------------------------------------------
TUN_S(model) = @time TUN_SEARCH(model);
TUN_R(model) = @time TUN_RESULT(model);
TUN_W(model) = @time TUN_WRITE(model);
#---------------------------------------------------------
print("fun.ok");
...
for id in 0:30
    ...
    @time begin
        #-------------------------------------------------------tables
        tb_places = CSV.File(string(id, ".Round.TimeDistance.csv")) |> DataFrame
        tb_cars = CSV.File(string(id, ".Round.Cars.csv")) |> DataFrame
        tb_requests = CSV.File(string(id, ".Round.Requests.csv")) |> DataFrame
        ...
        set_optimizer_attributes(model,"Threads" => 30
                                      ,"MIPFocus" => 2
                                      ,"ImproveStartTime" => 1800
                                      ,"TimeLimit" => 3600        
                                      ,"Method" => 3
                                      ,"Presolve" => 2
                                      ,"Cuts" => 3)
    ...
    SOL(model, id);
    VRW(model, requests, id);
    VRX(model, nodes, cars, id);
    VRY(model, nodes, cars, requests, id);
    VRZ(model, nodes, cars, id);
    VTA(model, nodes, cars, id);
    VTD(model, nodes, cars, id);=#
end
